...
impl Operation {
    fn execute(&self, target: &mut i32) -> i32 {
        match self {
...
impl Instruction {
    fn execute(&self, registers: &mut Registers) -> Option<i32> {
        if self.condition.evaluate(registers) {
            let target = registers.entry(self.target.clone()).or_insert(0);
            Some(self.operation.execute(target))
        } else {
            None
        }
    }
...
pub fn max(input: &[String]) -> i32 {
    ...
    input.into_iter()
        ...
        .filter_map(|instruction| instruction.execute(&mut registers))
        .max().unwrap()
}
