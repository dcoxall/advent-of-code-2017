...
pub fn dance(cycles: usize, moves: &[&str], dancers: &mut [char]) -> () {
    let mut iterations: usize = 0;
    let mut resulting_formations: HashSet<String> = HashSet::new();
    let mut formation: String = dancers.iter().collect();
    resulting_formations.insert(formation);
    ...
    while iterations < cycles {
        ...
        iterations += 1;
        formation = dancers.iter().collect();
        if resulting_formations.contains(&formation) {
            iterations = cycles - (cycles % iterations);
        } else {
            resulting_formations.insert(formation);
        }
    }
...
mod tests {
    ...
    fn executing_dance() {
        ...
        dance(2, &moves, &mut dancers);
        assert_eq!(dancers, vec!['c', 'e', 'a', 'd', 'b']);
    }
}
